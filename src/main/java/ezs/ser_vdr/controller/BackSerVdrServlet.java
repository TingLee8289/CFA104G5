...
@WebServlet("/ser_vdr/BackSerVdrServlet.do")
@MultipartConfig(fileSizeThreshold = 1024 * 1024, maxFileSize = 5 * 1024 * 1024, maxRequestSize = 5 * 5 * 1024 * 1024)
public class BackSerVdrServlet extends HttpServlet {
		...
		if ("getOne_For_Display".equals(action)) { // 查詢 findbyPK 來自select_page.jsp的請求
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					RequestDispatcher failureView = req.getRequestDispatcher("/backend/ser/serVdr/readSerVdr.jsp");// 參數要換
				...
				// Send the use back to the form, if there were errors
				if (!errorMsgs.isEmpty()) {
					RequestDispatcher failureView = req.getRequestDispatcher("/backend/ser/serVdr/readSerVdr.jsp");
					failureView.forward(req, res);
					return;// 程式中斷
				}
				...
				String url = "/backend/ser/serVdr/listOneSerVdr.jsp";// 結果顯示葉面
